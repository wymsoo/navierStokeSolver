...
if __name__ == "__main__":
    ...
    py = np.gradient(P, dy, axis=2)
    t_slice = slice(args.crop_t, U.shape[0])
    ...
    py_crop = py[t_slice, x_slice, y_slice]
    ...
    oper_dict_v, dictionary_v = build_v_ns_dict(
        u_crop, v_crop, vx_crop, vxx_crop, vy_crop, vyy_crop, py_crop
    )
    x_v = oper_dict_v[:]
    y_v = vt_crop.reshape(-1)
