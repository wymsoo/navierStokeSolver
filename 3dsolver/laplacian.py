...
def Laplacian(nx: int, ny: int, nz: int,
...
# Create 1D Laplacian operator in x-direction
    ...
    Iz = np.ones(nz)
    Dzz = diags([Iz, -2*Iz, Iz], [-1, 0, 1], shape=(nz, nz), format='csr')
    Dzz[0, 0] = -1
    Dzz[0, 1] = 1
    Dzz[-1, -2] = 1
    Dzz[-1, -1] = -1
    ...
    term1 = kron(Dxx, kron(Iy_sparse, Iz_sparse))
    term2 = kron(Ix_sparse, kron(Dyy, Iz_sparse))
